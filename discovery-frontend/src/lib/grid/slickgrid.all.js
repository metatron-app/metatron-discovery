...
(function ($) {
    ...
    function DataView(options) {
        ...
        // private
        var idProperty = "_idProperty_"; // property holding a unique row id
...
(function($) {
	function SlickColumnPicker(columns, grid, options) {
      ...
      updateColumnOrder();
			...
			for (var i = 0; i < columns.length; i++) {
				$li = $("<li />").appendTo($menu);
				$input = $("<input type='checkbox' />").data("column-id", columns[i]._idProperty_);
				columnCheckboxes.push($input);

				if (grid.getColumnIndex(columns[i]._idProperty_) != null) {
    ...
    function updateColumnOrder() {
      ...
      for (var i = 0; i < ordered.length; i++) {
        if ( grid.getColumnIndex(columns[i]._idProperty_) === undefined ) {
...
window.jQuery_1_7 = $.noConflict(true);
window.Slick = Slick;
        ...
        function markCopySelection(ranges) {
            ...
            for (var i = 0; i < ranges.length; i++) {
                for (var j = ranges[i].fromRow; j <= ranges[i].toRow; j++) {
                    hash[j] = {};
                    for (var k = ranges[i].fromCell; k <= ranges[i].toCell; k++) {
                        hash[j][columns[k]._idProperty_] = true;
                    }
                }
            }
    ...
    function AutoColumnSize(maxWidth) {
        ...
        function resizeAllColumns() {
            ...
            elHeaders.each(function(index, el) {
                ...
                var colIndex = grid.getColumnIndex(columnDef._idProperty_);
