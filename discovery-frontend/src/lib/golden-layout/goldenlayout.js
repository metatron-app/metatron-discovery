...
/*
      ...
      this._dragSources = [];
*/
      this.destroyDragSources();  // add by eltriny
    },
  ...
  lm.config.defaultConfig = {
    ...
    settings: {
      ...
      multiTabsEnabled: false,     // add by eltriny
      enableComponentResize: true,   // add by eltriny
      reorderEnabled: true,
  ...
  lm.utils.copy(lm.container.ItemContainer.prototype, {
    ...
    _$setSize: function (width, height) {
      if (width !== this.width || height !== this.height) {
        ...
        // ie bug fix - edit by eltriny
        var hdelta = 2;
        var vdelta = 2;
        if (-1 !== navigator.userAgent.toLowerCase().indexOf("chrome")) {
          hdelta = cl.offsetWidth - cl.clientWidth;
          vdelta = cl.offsetHeight - cl.clientHeight;
        }
  ...
  lm.controls.DragProxy = function (x, y, dragListener, layoutManager, contentItem, originalParent) {
    ...
    /* change by eltriny -- start */
    if ('' !== this._contentItem.config.componentState.imageUrl) {
      this.childElementContainer.append(
        '<img src="' + this._contentItem.config.componentState.imageUrl + '" style="width:100%;height:100%;"/>'
      );
    } else {
      this.childElementContainer.append(contentItem.element);
    }
    /* change by eltriny -- end */
  ...
  lm.utils.copy(lm.controls.DragSource.prototype, {
    ...
    _onDragStart: function (x, y) {
      if ('enable' === this._element.attr('data-gl-dragsource')) {  // add by eltriny - drag 비활성 처리를 위해서 추가
  ...
  lm.controls.Header._template = [
    '<div class="lm_header">',
    '<ul class="lm_tabs"></ul>',
    '<div class="lm_controls"></div>',      // edit by eltriny
...
*/
  ...
  lm.controls.Splitter = function (isVertical, size, grabSize, currConfig) {
    this._isVertical = isVertical;
    this._size = size;
    this._grabSize = grabSize < size ? size : grabSize;
    this._currConfig = currConfig;

    this.element = this._createElement();
    if (this._currConfig.settings.enableComponentResize) {  // edit by eltriny
      this._dragListener = new lm.utils.DragListener(this.element);
    } else {
      this.element.addClass('disable_pointer');
    }
  };
  ...
  lm.utils.copy(lm.items.Root.prototype, {
    ...
    setSize: function (width, height) {
      ...
      // add by eltriny
      ( 100 > width ) && ( width = 1588 );
      ( 100 > height ) && ( height = 800 );
  ...
  lm.utils.copy(lm.items.RowOrColumn.prototype, {
    ...
    _createSplitter: function (index) {
      var splitter;
      splitter = new lm.controls.Splitter(this._isColumn, this._splitterSize, this._splitterGrabSize, this.layoutManager.config);
      if (this.layoutManager.config.settings.enableComponentResize) {  // edit by eltriny
    ...
    _$highlightDropZone: function (x, y) {
      ...
      for (segment in this._contentAreaDimensions) {
        ...
        if (area.x1 < x && area.x2 > x && area.y1 < y && area.y2 > y) {
          if (segment === 'header') {
            // add by eltriny
            if (this.layoutManager.config.settings.multiTabsEnabled) {
