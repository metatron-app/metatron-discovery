...
export class AddDatasetComponent extends AbstractComponent implements OnInit, OnDestroy {
  ...
  public addDatasets() {
    ...
    const selectedDatasetsCloned = _.cloneDeep(this.selectedDatasets);
     ...
     // datasetLists 에 현재 화면에서 선택된 데이터셋 아이디와 데이터플로우에 이미 추가된 데이터셋을 합한다.
    let datasetLists =  [];
    selectedDatasetsCloned.forEach((ds) => {
      datasetLists.push(ds.dsId);
    });
    this.originalDatasetList.forEach((ds) => {
      datasetLists.push(ds.dsId);
    });

    this.loadingShow();
    this.dataflowService.updateDataSets(this.dataflowId, { dsIds : datasetLists })
      .then(() => {
        this.loadingHide();
        Alert.success(this.translateService.instant('msg.dp.alert.add.ds.success'));
        this.doneEvent.emit({isCancel:false});
      })
      .catch((error) => {
        this.loadingHide();
