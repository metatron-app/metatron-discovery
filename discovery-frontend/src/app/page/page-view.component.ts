    ...
    const paramSubs: Subscription = this.activatedRoute.queryParams.subscribe((params) => {
      ...
      if (params['pageId']) {
        this.widgetService.getWidget(params['pageId']).then(result => {
          this.setWidget = _.extend(new PageWidget(), result) as PageWidget;
        });
      } else {
    ...
    this.widget.dashBoard.configuration.fields.some((field: Field) => {
      if (field.name === changeField.name) {
        ...
        if (ChartType.MAP !== this.widgetConfiguration.chart.type) {
          ...
          PageViewComponent.updateShelfAliasFromField(this.widgetConfiguration.shelf, field, (this.widgetConfiguration.chart as UIMapOption).layerNum);
        }
        return true;
      }
    });
