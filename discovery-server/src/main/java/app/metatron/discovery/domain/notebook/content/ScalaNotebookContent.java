 ...
 */
public class ScalaNotebookContent extends NotebookContent {

    public ScalaNotebookContent() {
    }
    ...
    public String getSentence(String vars) {
        ...
        sentence.append("%spark").append(System.lineSeparator());
        ...
        for(String var : varsArr) {
            ...
            if(kv.length == 2) {
                ...
                if(key.startsWith(PREFIX_DATE_OPERATOR)) {
                    sentence.append("val " + key.substring(1) + " = \"" + super.getDate(kv[1]) + "\"");
                } else if(key.startsWith(PREFIX_NUMBER_OPERATOR)) {
                    sentence.append("val " + key.substring(1) + " = " + kv[1]);
                } else if(key.startsWith(PREFIX_STRARR_OPERATOR)) {
                    sentence.append("val " + key.substring(1) + " = \"'" + kv[1] + "'\"");
                } else {
                    sentence.append("val " + key + " = \"" + kv[1] + "\"");
                }
    ...
    public String getSentence(Map<String, Object> variables) {
        ...
        for ( String key : variables.keySet() ) {
            if(variables.get(key) instanceof Number) {
                ...
                sentence.append("val " + key + " = " + numValue).append(System.lineSeparator());
            } else {
                String value = (String) variables.get(key);
                if(key.startsWith("@")) {
                    value = super.getDate(value);
                }
                sentence.append("val " + key.replaceFirst("@","") + " = \"" + value + "\"").append(System.lineSeparator());
            }
        }
        return sentence.toString();
    }
