...
@JsonTypeName("time_single")
public class TimeSingleFilter extends TimeFilter {
  ...
  public TimeSingleFilter() {
    // Empty Constructor
  }

  @JsonCreator
  public TimeSingleFilter(@JsonProperty(value = "field", required = true) String field,
  ...
  public boolean compare(Filter filter) {
    if (!(filter instanceof TimeSingleFilter)) {
      return false;
    }

    TimeSingleFilter compareFilter = (TimeSingleFilter) filter;
