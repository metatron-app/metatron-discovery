 ...
 */
public class PyNotebookContent extends NotebookContent {

    public PyNotebookContent() {
    }
    ...
    public String getSentence(String vars) {
        ...
        sentence.append("%python").append(System.lineSeparator());
        String[] varsArr = vars.split(DELIMITER_SEMICOLON);
        for(String var : varsArr) {
            String[] kv = var.split(DELIMITER_EQUALITY);
            if(kv.length == 2) {
                String key = kv[0];
                if(key.startsWith(PREFIX_DATE_OPERATOR)) {     // Date format
                    sentence.append(key.substring(1) + " = \"" + super.getDate(kv[1]) + "\"");
                } else if(key.startsWith(PREFIX_NUMBER_OPERATOR)) {  // Number format
