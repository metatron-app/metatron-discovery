...
public class RoleSetProjections extends BaseProjections {
  ...
  public interface ForDetailViewProjection {
    ...
    //Set<Role> getRoles();
    @Value("#{T(app.metatron.discovery.util.ProjectionUtils).toListResource(@projectionFactory, T(app.metatron.discovery.domain.user.role.RoleProjections$RoleWithPermissionProjection), target.roles)}")
    List getRoles();
