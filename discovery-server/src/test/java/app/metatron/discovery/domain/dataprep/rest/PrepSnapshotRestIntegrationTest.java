...
public class PrepSnapshotRestIntegrationTest extends AbstractRestIntegrationTest {

    private static final Logger LOGGER = LoggerFactory.getLogger(PrepDataflowRestIntegrationTest.class);
    ...
    public String make_snapshot(PrepSnapshot.SS_TYPE ssType) {
        ...
        data.setEngine(PrepSnapshot.ENGINE.EMBEDDED);
        data.setSsType(ssType);
        data.setFormat(PrepSnapshot.FORMAT.CSV);
//        data.setCompression(PrepSnapshot.COMPRESSION.ZLIB);
    ...
    public void preparationsnapshots_GET_FILE_type() throws JsonProcessingException {
        String ssId = make_snapshot(PrepSnapshot.SS_TYPE.FILE);
        given()
                .auth()
                .oauth2(oauth_token)
                .accept(ContentType.JSON)
    ...
    public void preparationsnapshots_GET_HDFS_type() throws JsonProcessingException {
        String ssId = make_snapshot(PrepSnapshot.SS_TYPE.HDFS);
        given()
                .auth()
                .oauth2(oauth_token)
                .accept(ContentType.JSON)
